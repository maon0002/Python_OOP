class PizzaDelivery:
    ...
    def make_order(self) -> str:
        self.ordered = True
        order_ingredients = ', '.join(f"{i}: {q}" for i, q in self.ingredients.items())
        return f"You've ordered pizza {self.name} prepared with {order_ingredients} and the price will be {self.price}lv."
